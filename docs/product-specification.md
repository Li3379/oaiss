...
| 能力领域 | 说明 |
|---------|------|
| 碳核算管理 | 企业碳排放报告的创建、提交、审核全流程管理 |
| 碳配额交易 | 支持P2P点对点交易和双向拍卖两种交易模式 |
| 碳币交易市场 | 碳额度与碳币之间的兑换交易体系 |
| 信誉评分体系 | 基于交易行为和合规情况的企业信誉评估 |
| 数字签名 | RSA密钥对管理，碳报告签名验签，数据加密传输 |
| 碳中和项目 | 碳汇、CCUS、可再生能源等项目全生命周期管理 |
| 碳排放评级 | 基于排放数据的企业碳排放等级评定与行业排名 |
| 区块链存证 | 关键碳数据上链存证，确保不可篡改 |
| AI碳排放预测 | 基于历史数据的碳排放趋势预测 |
...
| ADMIN | 4 | 管理员 | 系统管理，用户管理，执行撮合，碳币充值，信誉评估 |
| THIRD_PARTY | 5 | 第三方监管 | 监管碳报告和交易数据，查看统计信息 |
...
碳币是系统内部的交易媒介，企业可以用人民币充值碳币，用碳币购买碳配额，或出售碳配额获得碳币。碳币支持用户间转账。
...
基于企业交易行为、合规情况、碳报告质量等维度综合评估企业信誉。信誉分影响企业交易权限：低于阈值被限制交易，严重不足被冻结账户。
...
- **扣分场景**: 碳报告审核拒绝、交易违规、数据造假等
- **加分场景**: 准时提交报告、交易诚信、参与碳中和项目等
...
碳中和项目全生命周期管理，支持碳汇、CCUS（碳捕集利用与封存）、可再生能源、节能改造等类型项目。项目经历创建、审核、实施、核证、认证、监测六个阶段。
...
基于企业年度碳排放数据进行等级评定，提供行业排名和AI碳排放趋势预测功能。评级结果影响企业的碳市场准入和交易限额。
...
> 本文档基于代码库自动生成，涵盖 OAISS Chain 系统全部 16 个后端控制器、79 个API接口、21 张数据库表、20 个前端页面的完整业务说明。
