...
vi.mock('../../api/carbonCoin', () => ({
  getMyAccount: vi.fn(() => Promise.resolve({ data: { balance: 100 } })),
...
describe('CarbonCoin.vue', () => {
  ...
  it('API调用失败显示错误消息', async () => {
    getMyAccount.mockRejectedValueOnce(new Error('network error'))
    const wrapper = mountComponent()
    await flushPromises()
  ...
  it('组件渲染数据', async () => {
    getMyAccount.mockResolvedValueOnce({ data: { balance: 100 } })
    const wrapper = mountComponent()
    await flushPromises()
    expect(getMyAccount).toHaveBeenCalled()
    wrapper.unmount()
